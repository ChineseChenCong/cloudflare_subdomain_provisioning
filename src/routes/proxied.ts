...
proxied.put("/records/:recordId/proxied", async (c) => {
  ...
  // 获取子域名
  const subdomain = await getSubdomainById(c.env, record.subdomain_id);
  if (!subdomain) {
  ...
  try {
    ...
    if (!resolved) {
      return c.json(
        {
          error: `无法解析域名 ${subdomain.domain} 的 Zone ID，请检查账户权限`,
        },
        400,
      );
    }
    ...
    // write-through：镜像侧同步该 DNS 记录行（best-effort，失败静默，日级 cron 回补）
    await mirrorSyncRow(c.env, "dns_records", record.id).catch(() => {});

    return c.json({
      success: true,
      proxied: body.proxied,
      message: body.proxied
        ? "已开启代理（黄色云朵）"
        : "已关闭代理（灰色云朵）",
    });
  } catch (err: any) {
...
proxied.put("/subdomains/:subdomainId/records/proxied", async (c) => {
  ...
  // 获取子域名
  const subdomain = await getSubdomainById(c.env, subdomainId);
  if (!subdomain) {
    return c.json({ error: "子域名不存在" }, 404);
  }
  ...
  // 获取所有 DNS 记录
  const { getSubdomainRecords, updateDnsRecordEntry } =
    await import("../db/queries");
  const records = await getSubdomainRecords(c.env, subdomainId);

  // 筛选支持代理的记录
  const supportedRecords = records.filter((r) =>
    ["A", "AAAA", "CNAME"].includes(r.record_type),
  );

  const targetRecords =
    body.record_ids && body.record_ids.length > 0
