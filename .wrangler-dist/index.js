...
async function createDnsRecordEntry(db, subdomainId, cfRecordId, recordType, name, content, ttl, priority, proxied2, comment) {
  ...
  if (!record)
    throw new Error("Failed to create DNS record entry");
  return record;
}
...
async function createCloudflareAccount(db, userId, accountName, encryptedApiToken, zoneId, isDefault) {
  ...
  if (!account)
    throw new Error("Failed to create cloudflare account");
  return account;
}
async function updateCloudflareAccount(db, userId, accountId, updates) {
  ...
  if (!updated)
    throw new Error("Failed to update cloudflare account");
  return updated;
}
...
async function getActiveAnnouncements(db) {
  const result = await db.prepare("SELECT * FROM announcements WHERE is_active = 1 ORDER BY created_at DESC, id DESC").all();
  return result.results;
}
async function getAllAnnouncements(db) {
  const result = await db.prepare("SELECT * FROM announcements ORDER BY created_at DESC, id DESC").all();
  return result.results;
}
async function createAnnouncement(db, title, content, createdBy) {
  const result = await db.prepare(
    "INSERT INTO announcements (title, content, is_active, created_by) VALUES (?, ?, 1, ?)"
  ).bind(title, content, createdBy).run();
  const id = result.meta.last_row_id;
...
// src/config.ts
var zoneIdCache = /* @__PURE__ */ new Map();
...
api.post("/subdomains", async (c) => {
  ...
  try {
    ...
    for (const admin2 of adminList.filter((u) => u.is_admin && u.email)) {
  ...
  if (subdomain.status === "approved") {
    ...
    if (zoneId) {
      ...
      for (const record of records) {
        try {
          await deleteDnsRecord(c.env.CF_API_TOKEN, zoneId, record.cf_record_id);
        } catch (err) {
...
api.post("/subdomains/:id/records", async (c) => {
  ...
  try {
    const cfRecord = await createDnsRecord(c.env.CF_API_TOKEN, zoneId, {
      type: body.type,
      name: body.name || "@",
  ...
  <script>
    ...
    async function saveAnnouncement() {
      ...
      try {
        ...
        if (editing) {
          await api('/announcements/admin/' + editing.id, { method: 'PUT', body: JSON.stringify({ title, content }) });
          toast('\u516C\u544A\u5DF2\u66F4\u65B0', 'success');
        } else {
          await api('/announcements/admin', { method: 'POST', body: JSON.stringify({ title, content }) });
          toast('\u516C\u544A\u5DF2\u53D1\u5E03', 'success');
        }
        state.editingAnnouncement = null;
        await loadAdminData();
    ...
    async function toggleAnnouncement(id) {
      ...
      if (!a) return;
      try {
        await api('/announcements/admin/' + id, { method: 'PUT', body: JSON.stringify({ is_active: !a.is_active }) });
    ...
    async function deleteAnnouncement(id, title) {
      showModal('\u5220\u9664\u516C\u544A', '\u786E\u5B9A\u5220\u9664\u516C\u544A\u300C' + title + '\u300D\u5417\uFF1F', async () => {
        try {
          await api('/announcements/admin/' + id, { method: 'DELETE' });
          toast('\u516C\u544A\u5DF2\u5220\u9664', 'success');
          await loadAdminData();
          render();
        } catch (err) { toast(err.message, 'error'); }
...
<body>
  ...
  <footer class="footer">
    <div class="container">
      `, '\n      <p>Powered by Cloudflare Workers & D1 \xB7 SubDomain Hub</p>\n      <p style="margin-top:8px;font-size:12px;color:var(--text-muted);">\n        \u611F\u8C22 <a href="https://github.com/Little100/cloudflare_subdomain_provisioning" target="_blank" rel="noopener">Little100/cloudflare_subdomain_provisioning</a> \u5F00\u6E90\u9879\u76EE\n      </p>\n      ', "\n      ", "\n    </div>\n  </footer>\n</body>\n</html>"])), siteName, backgroundOverlay || "rgba(15, 23, 42, 0.7)", backgroundImage ? `<div class="bg-image-wrapper"><img class="bg-image" src="${backgroundImage}" alt="background" /><div class="bg-overlay"></div></div>` : "", siteLogo ? `<img src="${siteLogo}" alt="logo" class="logo-icon" style="width:36px;height:36px;object-fit:contain;background:none;box-shadow:none;border-radius:12px;" />` : `<div class="logo-icon">${defaultLogoSvg}</div>`, siteName, user ? `JSON.parse('${JSON.stringify({ id: user.id, github_username: user.github_username, avatar_url: user.avatar_url, is_admin: !!user.is_admin, email: user.email, email_verified: user.email_verified })}')` : "null", friendLinks && friendLinks.length > 0 ? `
...
async function createAccount(db, env, userId, accountName, apiToken, zoneId) {
  ...
  if (!finalZoneId && env.DOMAINS) {
    ...
    if (domains.length > 0) {
      const zoneId2 = await resolveZoneId2(apiToken, domains[0]);
      if (zoneId2) {
        finalZoneId = zoneId2;
      }
    }
  ...
  const result = await db.prepare(
    `INSERT INTO cloudflare_accounts (user_id, account_name, api_token, zone_id, is_active, is_default)
       VALUES (?, ?, ?, ?, 1, ?)`
  ).bind(userId, accountName, encryptedToken, finalZoneId, isDefault ? 1 : 0).run();
  const id = result.meta.last_row_id;
...
async function resolveZoneId2(apiToken, domain) {
  try {
    const response = await fetch(
      `https://api.cloudflare.com/client/v4/zones?name=${encodeURIComponent(domain)}&status=active`,
      {
...
__name(resolveZoneId2, "resolveZoneId");
async function resolveZoneIdAndTokenFromAccounts(accounts2, domain) {
  ...
  for (const { token } of accounts2) {
    const zoneId = await resolveZoneId2(token, domain);
    if (zoneId) {
      return { zoneId, token };
    }
  }
...
admin.post("/", async (c) => {
  const user = c.get("user");
  const body = await c.req.json().catch(() => ({ title: "", content: "" }));
...
admin.put("/:id", async (c) => {
  const id = parseInt(c.req.param("id"), 10);
  const body = await c.req.json().catch(() => ({ title: void 0, content: void 0, is_active: void 0 }));
...
app.route("/api/announcements", announcements_default);
